...
// Initialize database connection & express app
async function startServer() {
  try {
    if (process.env.DB_ACTIVE === "true") {
      await initDatabase(); // On attend la création des tables
    }
    ...
    if (UPTIME_ROBOT_API_KEY_MAIN) {
      ...
      app.get("/api/account-details", async (req, res, next) => {
        ...
        try {
          ...
          logger.error("Error in /api/account-details", {
            error: error.message,
          });
          next(error);
        }
    ...
    if (UPTIME_ROBOT_API_KEY_READ_ONLY) {
      ...
      app.get("/api/monitor/:monitorId", async (req, res, next) => {
        ...
        try {
          ...
          const responseTimesStartDate = Math.floor(
